...
The same output contract now applies across rumen, wetland/MUCC, mangrove/MSM,
mangrove/Futian, and MUCC v1 Old Woman Creek lanes, with lane-specific
denominators and functional evidence contracts:
...
| Mangrove/Futian expansion gate | 3,404 phase-1 Futian rMAGs; 3,156 ready payload rows; 248 gap rows | ESM2 and gLM2 complete for 3,156 ready payloads; functional shards consolidated by manifest with archaea, bacteria, gap, failed, partial, duplicate, and not-started status rows |
| MUCC v1 source-scaffold gate | 2,508 archive MAGs | 2,501 ESM-2, 2,508 gLM2 and source-functional payloads, source DRAM/gene/expression evidence retained under its distinct contract, and exact sample-linkage gaps explicit |
| Common mechanism contract gate | all lanes proposed for one quantitative mechanism rank | methane, sulfur, and substrate features rebuilt from one accepted/present event contract with duplicate-hit, denominator, missingness, and source-balance validation |
...
For any report that mixes POC, MSM, and Futian lanes, the report manifest must
record the lane-registry status snapshot or a freeze manifest generated by
`scripts/reports/build_methanet_3view_payload_freeze.py`.

The July 24 release admits 2,931/3,156 Futian ready rows with
annotation-complete functional payloads. It preserves 225 ready rows without
that payload and 248 source-gap rows. The common mechanism contract gate
remains open across MSM, Futian, and MUCC v1.
